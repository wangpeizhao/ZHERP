//
//  WLocationOperateViewController
//  ZHERP
//
...
class WLocationOperateViewController: UIViewController {
    ...
    let dataArr = [
        ["title":"库位名称", "key":"name", "value":"未填写"],
        ["title":"所属仓库", "key":"warehouse", "value":"未选择"]
    ]
    ...
    @objc func actionSave() {
        for (_, item) in self.valueArr {
            if item.isEmpty {
                _alert(view: self, message: "请完整填写库位信息")
                return
            }
        }
...
extension WLocationOperateViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case "warehouse":
            detailTextLabel = !(self.valueArr["warehouse"]?.isEmpty)! ? self.valueArr["warehouse"]! : _data["value"]!
        default:
            detailTextLabel = ""
        }
